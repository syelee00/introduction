...
type Card = { label: string; icon: string; description: string; accent: string; link?: string; gif?: string };
type Group = { title: string; subtitle: string; hint?: string; cards: Card[] };
...
const groups: Group[] = [
  ...
  { title: "새롭게 만드는 걸\n좋아합니다", subtitle: "MAKE IT FROM ZERO", cards: [
    { label: "AI 서비스", icon: "✦", description: "웨딩 시안을 제작해 주는 웹을 만들고 보완하고 있어요.", accent: "blue", link: "https://pre-shot-seven.vercel.app/" },
    { label: "이모티콘", icon: "☺", description: "이 시리즈 이모티콘을\n만들어 봤어요 ㅎㅎ", accent: "yellow", gif: "/emoticon.gif" },
...
function InterestCard({ card }: { card: Card }) {
  ...
  return <button className={`flip-card ${flipped ? "is-flipped" : ""}`} onClick={() => setFlipped(!flipped)} aria-label={`${card.label} ${flipped ? "설명 닫기" : "설명 보기"}`}><span className="flip-inner"><span className={`flip-face front ${card.accent}`}><b>{card.icon}</b><em>{card.label}</em><small>CLICK ME</small></span><span className={`flip-face back ${card.accent}`}><strong>{card.label}</strong>{card.gif ? <><img className="emoticon-gif" src={card.gif} alt="이모티콘 애니메이션" /><span className="emoticon-copy">{card.description}</span></> : <span>{card.description}</span>}{card.link && <a className="card-link" href={card.link} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>사이트 보러 가기 ↗</a>}</span></span></button>;
}
...
export default function Home() {
  ...
  return <main>
    <section className="hero" id="about"><header className="topbar"><a href="#about" className="mark">SY</a></header><p className="eyebrow page-label">INTRODUCTION</p><h1 className="intro-title"><span>안녕하세요,</span><span>저는 <i>이소연</i>입니다</span></h1><div className="portrait-wrap"><div className="portrait"><img src="/soyeon-illustrated-v2.png" alt="손을 들고 미소 짓는 이소연 일러스트" /></div><span className="tape tape-a" /><span className="tape tape-b" /></div><div className="paper-note intro-note">새로운 걸 배우고,<br />해보는 걸 좋아하는 사람</div><div className="interest-grid">{groups.map((group, index) => <section className={`interest-group group-${index + 1}`} key={group.subtitle}><div className="group-heading">{group.hint && <p className="card-hint">{group.hint}</p>}<h2>{group.title.split("\n").map((line) => <span key={line}>{line}</span>)}</h2><p>{group.subtitle}</p></div><div className="card-row">{group.cards.map((card) => <InterestCard key={card.label} card={card} />)}</div></section>)}</div><p className="scroll-note">SCROLL TO READ MY NOTES ↓</p></section>
    <section className="dream-section" id="dream"><p className="eyebrow dream-label">DWNC에서 만들고 싶은 것</p><h2>운동이 조금 더<br /><em>기다려지는</em> 앱</h2><div className="idea-cloud"><span>날씨 기반 코스</span><span>신호 최소화 경로</span><span>이동형 알람</span><span>러닝 대결</span></div><p className="dream-copy">혼자여도 계속 달릴 수 있도록 하는 앱을 만들고 싶어요.{"\n\n"}함께 새로운 아이디어를 공유하고, 함께 개발하고 싶어요!</p></section>
    <section className="contact-section"><div><p className="eyebrow">LET&apos;S SAY HELLO</p><h2>같이 재밌는 걸<br />만들어봐요.</h2></div><div className="contact-links"><a href="mailto:syelee00@snu.ac.kr">EMAIL · syelee00@snu.ac.kr ↗</a><a href="https://www.instagram.com/soyyyeun/" target="_blank" rel="noreferrer">INSTAGRAM · soyyyeun ↗</a></div></section>
